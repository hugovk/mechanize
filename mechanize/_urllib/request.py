...
from .error import ContentTooShortError
...
# Prevent a circular import between Mechanize's _request.py and
# urllib.request. Import only on demand
_request = None
...
timeout=socket._GLOBAL_DEFAULT_TIMEOUT):
    ...
    def error(self, proto, *args):
        if proto in ('http', 'https'):
            ...
            dict = self.handle_error['http'] # https is not different
                                             # than http
            proto = args[2]  # YUCK!
...
class HTTPRedirectHandler(BaseHandler):
    ...
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        ...
        m = req.get_method()
        if (not (code in (301, 302, 303, 307, "refresh") and m in ("GET", "HEAD")
...
class ProxyHandler(BaseHandler):
    ...
    def __init__(self, proxies=None, proxy_bypass=proxy_bypass):
        if proxies is None:
            proxies = getproxies()
...
class ProxyBasicAuthHandler(AbstractBasicAuthHandler, BaseHandler):
    ...
    def http_error_407(self, req, fp, code, msg, headers):
        # http_error_auth_reqed requires that there is no userinfo
        # component in authority.  Assume there isn't one, since
        # urllib.request does not (and should not, RFC 3986 s. 3.2.1)
        # support requests for URLs containing userinfo.
        authority = req.host
...
class FancyURLopener(URLopener):
    ...
    def http_error_307(self, url, fp, errcode, errmsg, headers, data=None):
        ...
        if data is None:
            return self.http_error_302(url, fp, errcode, errmsg, headers,
data)
        else:
            return self.http_error_default(url, fp, errcode, errmsg, headers)
